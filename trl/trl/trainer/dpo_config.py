...
class DPOConfig(TrainingArguments):
    ...
    #
    # When enabled, we compute K objective-specific DPO losses:
    #   L_i = - mean_b log σ( β * s_{i,b} * Δ_b )
    # where
    #   Δ_b = (logπ(y1|x)-logπ(y2|x)) - (logπ_ref(y1|x)-logπ_ref(y2|x))
    # and s_{i,b} ∈ {+1,-1} indicates whether objective i prefers y1 (chosen) or y2 (rejected).
    #
    # Supported setups:
    #   - 2 objectives: quality + verbosity
    #   - 3 objectives: quality + verbosity + faithfulness
    # ---------------------------------------------------------------------
    raco: bool = field(
        default=False,
        metadata={"help": "Enable RACO multi-objective DPO loss."},
    )
    raco_weights: list[float] = field(
        default_factory=lambda: [0.8, 0.2],
        metadata={"help": "Objective weights for RACO. Supports 2 objectives (quality, verbosity) or 3 objectives (quality, verbosity, faithfulness)."},
    )
    raco_log_grad_norms: bool = field(
        default=False,
        metadata={
            "help": "If True, compute per-objective gradient norms (via autograd.grad) and log them. "
            "This is useful for debugging and for later CGrad steps, but adds overhead."
        },
    )
    raco_verbosity_prefer_shorter: bool = field(
        default=True,
        metadata={"help": "If True, verbosity objective prefers shorter completion; otherwise prefers longer."},
    )
    raco_c: float = field(
        default=0.4,
        metadata={"help": "CGrad/CAGrad constant c used in RACO. Default 0.4."},
    )
    raco_use_cagrad: bool = field(
        default=True,
        metadata={"help": "If True, apply the CGrad/CAGrad projection step after forming g0. If False, use g0."},
    )
    raco_clip_lambda: bool = field(
        default=False,
        metadata={
            "help": "If True, clip CAGrad mixing coefficients p_i to [0, w_i]. "
            "This ensures p_quality <= w_quality and p_verbosity <= w_verbosity."
        },
    )
    raco_length_normalized: bool = field(
        default=False,
        metadata={
            "help": "If True, length-normalize per-sequence log-probs before forming the DPO log-ratio delta in RACO. "
            "Concretely, use avg logp (logp/|y|) for chosen/rejected for both policy and reference models."
        },
    )

    # ---------------------------------------------------------------------
    # Baseline selector (used by our training scripts)
    # ---------------------------------------------------------------------
    mode: str = field(
        default="raco",
        metadata={"help": "Training baseline mode. Supported: 'raco' or 'amopo' (static AMoPO Eq.9)."},
    )
    ...
    def __post_init__(self):
        ...
        # Validate RACO config
        if getattr(self, "raco", False):
            if not isinstance(self.raco_weights, list) or len(self.raco_weights) not in {2, 3}:
                raise ValueError(
                    f"`raco_weights` must be a list of length 2 or 3, got: {self.raco_weights}"
                )
            s = sum(float(x) for x in self.raco_weights)
            if s <= 0:
                raise ValueError(f"`raco_weights` must sum to > 0, got sum={s} from {self.raco_weights}")
            if float(self.raco_c) < 0:
                raise ValueError(f"`raco_c` must be >= 0, got {self.raco_c}")
        super().__post_init__()
